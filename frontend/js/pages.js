...
async function loadLearnPage(targetPage) {
    ...
    try {
        const response = await fetch('pages/learn.html?v=20260831c', { cache: 'no-store' });
        if (response.ok) {
    ...
    <section id="algoritmos">
        ...
        <table class="data-table">
            ...
            <tbody>
                ...
                <tr><td>Insiders <span class="text-muted" style="font-size:11px;">(nuevo)</span></td><td>MSPR (Finnhub): compra/venta neta de directivos, último mes</td><td>Más positivo (compra neta) = mejor</td><td>Solo acciones</td></tr>
...
en medio                                → NEUTRAL</pre>
        ...
        <h3>⑪ Fundamentales (SOLO acciones)</h3>
        <p><strong>Qué mide:</strong> hasta aquí todo era <em>precio</em> (tendencia, riesgo, técnico). Pero una acción también <strong>es un negocio</strong>, y un negocio se analiza con sus números. Para las acciones individuales (los ETFs, fondos y bonos <em>no</em> tienen estos datos) bajamos los fundamentales reales de la empresa y los agrupamos en <strong>cinco jueces</strong>:</p>
        <ul>
            <li><strong>Valoración</strong> — ¿está barata? PER, precio/valor contable (P/B), PEG. <em>Cuanto más bajo, mejor.</em></li>
            <li><strong>Calidad</strong> — ¿es buen negocio? ROE (rentabilidad sobre recursos propios), márgenes, generación de caja (FCF). <em>Cuanto más alto, mejor.</em></li>
            <li><strong>Crecimiento</strong> — ¿crece? Crecimiento de ventas y de beneficios.</li>
            <li><strong>Solidez</strong> — ¿aguanta? Deuda sobre recursos propios (menos es mejor) y liquidez corriente.</li>
            <li><strong>Insiders</strong> <span class="text-muted" style="font-size:12px; font-weight:normal;">(añadido 14-sep-2026)</span> — ¿qué hace la propia directiva con su dinero? MSPR de Finnhub: compra/venta neta de acciones por parte de directivos e insiders en el último mes reportado. <em>Cuanto más positivo (compra neta), mejor.</em></li>
        </ul>
        <p><strong>Detalle de rigor clave — valoración normalizada por sector:</strong> el PER de un banco (típicamente ~8-12) no es comparable al de una tecnológica (~25-40). Por eso la valoración se puntúa <strong>dentro de cada sector</strong>: un banco compite contra bancos, no contra Nvidia. Así no recomendamos "bancos baratos" sobre "tech cara" a ciegas.</p>
        <p><strong>Por qué se añadió el juez de insiders:</strong> los tres jueces de arriba (valoración, calidad, crecimiento, solidez) miran <em>los números publicados</em> del negocio. Pero hay una señal distinta: qué hace con su propio dinero la gente que mejor conoce la empresa por dentro. Compra neta de insiders no garantiza nada, pero es una de las pocas señales con evidencia académica real de anticipar rendimiento (Lakonishok &amp; Lee, 2001) — por eso entra como quinto juez, con peso moderado, nunca como señal única.</p>
        <p><strong>Cómo se combina:</strong> para una acción, estos jueces se mezclan con los de precio (60% precio / 40% fundamentales): <em>valoración (45%) + solidez (35%) + insiders (20%)</em> suman a la tesis <strong>VALOR</strong>, y <em>calidad (60%) + crecimiento (40%)</em> suman a <strong>MOMENTUM</strong>. Resultado: si los números dicen que una empresa es buena y barata, y además su directiva está comprando, <strong>sube en el ranking y se recomienda</strong>, con sus ratios visibles en la tarjeta para justificarlo. Los ETFs/fondos/bonos siguen puntuando solo por precio, como siempre.</p>
        <p><strong>Honestidad sobre los datos:</strong> los fundamentales vienen de Yahoo (yfinance) y el insider sentiment de Finnhub (plan gratuito) — ambos gratuitos pero no perfectos. Algunos sectores (bancos, aseguradoras) no reportan ciertos campos (FCF, deuda); algunas acciones pequeñas o poco líquidas no tienen datos de insiders. En cualquiera de esos casos ese juez simplemente no vota para esa empresa, en vez de inventarse un número. Además, una venta de insider no siempre es señal bajista (puede ser diversificación personal o ejercicio de opciones) — por eso solo se puntúa la compra neta con peso moderado, no cada transacción individual.</p>
        ...
        <h3>⑬ Entorno/catalizadores reales + horizonte, en cualquier activo buscado <span class="text-muted" style="font-size:12px; font-weight:normal;">(añadido 21-sep-2026)</span></h3>
        <p><strong>Qué es:</strong> hasta ahora, esto solo existía para tus posiciones en "¿Vender o mantener?": una búsqueda de <strong>noticias reales y recientes</strong> (Google News RSS, sin necesitar clave ni registro) sobre la empresa concreta, resumida por la IA en un veredicto — <strong>favorable / neutral / adverso</strong> — con catalizadores, riesgos y las fuentes citadas. Ahora también aparece automáticamente en el <strong>"análisis profesional de activo"</strong> (el que abres al buscar cualquier ticker), para no tener que pedirlo aparte cada vez.</p>
        <p><strong>Honestidad de diseño:</strong> si la búsqueda no encuentra nada relevante, <strong>no aparece ningún veredicto inventado</strong> — el motor nunca rellena con la memoria genérica del modelo, solo sintetiza lo que hay en titulares reales y fechados.</p>
        <p><strong>Horizonte que mejor encaja:</strong> además, cada análisis indica si el perfil actual del activo encaja mejor con la tesis <strong>momentum (corto-medio plazo, ~1-3 meses)</strong> o con la tesis <strong>valor/reversión (medio-largo plazo, ~6-18 meses)</strong> — usando los mismos criterios que ya puntúan el motor (por encima de su media de 200 sesiones + momentum positivo = momentum; muy por debajo de su propia media de 50 sesiones = valor). Si está muy estirado sobre su media de 200 sesiones (≥30%), se avisa explícitamente: no es una predicción de que vaya a corregir, es un hecho estadístico general (cuanto más lejos de su media, más probable un respiro) que aplica más allá de cuál sea su tesis.</p>
        <p><strong>📖 ¿Qué es este activo? <span class="text-muted" style="font-size:12px; font-weight:normal;">(añadido 21-sep-2026)</span>:</strong> tanto en "Analiza cualquier activo" como en el análisis profesional aparece ahora una breve descripción real de qué es la empresa/fondo/cripto, a qué se dedica o qué índice replica, sector/categoría y, para cripto, contexto histórico (año de lanzamiento). Es el propio resumen de Yahoo Finance, nunca texto generado por la IA — si Yahoo no tiene resumen para ese ticker (listados muy pequeños u obscuros), simplemente no aparece el bloque en vez de inventar uno.</p>
        ...
        <p><strong>También en la tabla de ranking cuantitativo <span class="text-muted" style="font-size:12px; font-weight:normal;">(añadido 21-sep-2026)</span>:</strong> cada fila del escaneo completo (no solo las ideas curadas por la IA) lleva ahora una columna <strong>Horizonte</strong> con una insignia — 🔥 corto (1-3m) o 🧊 medio-largo (6-18m) — calculada comparando los mismos dos scores que ya ves en esa fila (Mom. vs Valor), así que nunca puede contradecir lo que la tabla ya muestra. Si ninguno de los dos scores es claramente positivo, aparece "sin encaje claro" en vez de forzar una etiqueta.</p>
    ...
    <section id="glosario">
        ...
        <table class="data-table">
            ...
            <tbody>
                ...
                <tr><td><strong>Entorno/catalizadores (web)</strong></td><td>Noticias reales y recientes sobre una empresa concreta (Google News, sin clave), resumidas por la IA en un veredicto favorable/neutral/adverso con fuentes citadas. Nunca inventa: sin noticias relevantes, no hay veredicto. Ver <a href="#algoritmos">algoritmos ⑬</a>.</td></tr>
                ...
                <tr><td><strong>Fundamentales</strong></td><td>Los números del negocio de una acción (no del precio): valoración (PER, P/B, PEG), calidad (ROE, márgenes, caja), crecimiento y solidez (deuda). Solo aplican a acciones individuales, no a ETFs/fondos. Ver <a href="#algoritmos">algoritmos ⑪</a>.</td></tr>
                ...
                <tr><td><strong>Horizonte</strong></td><td>La ventana de tiempo típica de una estrategia: momentum 1-3 meses, valor 6-18 meses. Coincide con los plazos que mide el scorecard. El análisis de cualquier activo buscado, y ahora también cada fila del ranking cuantitativo en Oportunidades (insignia 🔥/🧊), muestra con cuál de las dos encaja mejor su perfil actual. Ver <a href="#algoritmos">algoritmos 🧭</a> y <a href="#algoritmos">⑬</a>.</td></tr>
                <tr><td><strong>Insiders (compras de directivos)</strong></td><td>MSPR (Monthly Share Purchase Ratio) de Finnhub: si los directivos de una empresa compraron o vendieron más acciones propias el último mes, en escala -100 a +100. Compra neta positiva es una señal de confianza de quien mejor conoce el negocio. Ver <a href="#algoritmos">algoritmos ⑪</a>.</td></tr>
                ...
                <tr><td><strong>PER (P/E)</strong></td><td>Precio dividido entre beneficio por acción: cuántos años de beneficios actuales "pagas" al comprar. Más bajo = más barato, pero solo comparable dentro del mismo sector. Ver <a href="#algoritmos">algoritmos ⑪</a>.</td></tr>
                ...
                <tr><td><strong>ROE (rentabilidad sobre recursos propios)</strong></td><td>Beneficio que genera una empresa por cada euro de capital de sus accionistas. Más alto = negocio más rentable/eficiente. Juez de "calidad". Ver <a href="#algoritmos">algoritmos ⑪</a>.</td></tr>
                ...
                <tr><td><strong>Tamaño de posición (volatilidad inversa)</strong></td><td>Cuánto peso dar a una idea calculado al revés de su volatilidad (más movimiento, menos peso), y ajustado además por su correlación con tu cartera: menos peso si añade concentración, más si diversifica. Ver <a href="#algoritmos">algoritmos 🧭</a> y <a href="#motor-sistematico">El motor sistemático</a>.</td></tr>
